...
- **Working POC, and Peter likes the layout.** Two scripts, currently in the
  aifabric strand's inbox (`aifabric/ideas/`):
  - `aifabric-tmux-poc.sh` — builds the 3-zone tmux session `plane` from OUTSIDE
    the tmux CLI (never attaches; captures pane IDs so layout is renumber-proof).
    Verified layout: OVERVIEW top / two keeper panes middle / DRIVER bottom.
  - `aifabric-tmux-overview.sh` — the summary readout (repaints every 5s).
- **View it:** `tmux attach -t plane` in a full-screen terminal (build first if
  gone: run `aifabric-tmux-poc.sh`). Remove: `tmux kill-session -t plane`.
...
- Per-strand identity: coloured borders + titles (transparency can't tint
  ...
  `colour` file).
- Where do the POC scripts finally live — promote from `aifabric/ideas/` into
